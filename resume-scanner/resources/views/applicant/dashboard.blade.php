...
<div class="space-y-6">
    <section class="relative overflow-hidden rounded-2xl bg-gradient-to-r from-blue-600 via-blue-700 to-indigo-700 p-6 text-white shadow-lg">
        <div class="relative z-10">
            ...
            <h1 class="text-2xl sm:text-3xl font-bold">{{ __('messages.welcome_back') }}, {{ $firstName }} 👋</h1>
            <p class="mt-1 text-blue-200 text-sm max-w-2xl">{{ __('messages.dashboard_intro', ['count' => $totalApps, 'application_word' => __($totalApps === 1 ? 'messages.application_singular' : 'messages.application_plural')]) }}</p>
            <div class="mt-4">
                <a href="{{ route('applicant.jobs') }}" class="inline-flex items-center gap-1.5 rounded-xl bg-white text-blue-700 font-semibold text-sm px-4 py-2 hover:bg-blue-50 transition-colors">{{ __('messages.browse_vacancies') }}</a>
            </div>
        </div>
    ...
    <section class="rounded-2xl border border-slate-200 bg-white shadow-sm dark:border-slate-700 dark:bg-slate-800">
        <div class="flex items-center gap-2 border-b border-slate-200 px-6 py-4 dark:border-slate-700">
            ...
            <h2 class="text-xl font-bold tracking-wide text-slate-800 dark:text-slate-100">{{ Illuminate\Support\Str::upper(__('messages.application_tips')) }}</h2>
        </div>
        ...
        <div class="grid gap-4 p-6 md:grid-cols-2">
            <article class="rounded-xl bg-blue-100/80 p-5 dark:bg-blue-900/30">
                <div class="mb-3 flex items-center gap-3">
                    ...
                    <h3 class="text-xl font-semibold text-slate-800 dark:text-slate-100">{{ __('messages.complete_your_profile') }}</h3>
                </div>
                <p class="text-xl leading-7 text-slate-700 dark:text-slate-200">{{ __('messages.complete_profile_help') }}</p>
            </article>

            <article class="rounded-xl bg-blue-100/80 p-5 dark:bg-blue-900/30">
                <div class="mb-3 flex items-center gap-3">
                    <span class="inline-flex h-8 w-8 items-center justify-center rounded-full bg-blue-800 text-sm font-bold text-white">2</span>
                    <h3 class="text-xl font-semibold text-slate-800 dark:text-slate-100">{{ __('messages.apply_vacancy_online') }}</h3>
                </div>
                <p class="text-xl leading-7 text-slate-700 dark:text-slate-200">{{ __('messages.apply_vacancy_online_help') }}</p>
            </article>
        </div>

        <div class="px-6 pb-5 text-xl text-slate-700 dark:text-slate-200">
            {{ __('messages.feedback_help') }}
            <a href="{{ route('privacy') }}" class="font-semibold text-blue-700 hover:underline dark:text-blue-300">{{ __('messages.here') }}</a>.
        </div>
    </section>
</div>
@endsection
